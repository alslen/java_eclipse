...
ServerSocket s;
Socket s1;
DataOutputStream dos;  // 데이터를 출력
DataInputStream dis;  // 데이터 읽을 때
boolean stop;
// 필요한 변수를 정의.
...
public void service() {
	try{
		...
		dis = new DataInputStream(s1.getInputStream());  //클라이언트가 입력한 내용을 읽어서 저장함
...
//actionPerformed() 메소드를 구현한다. 
	...
	try {
			String msg = tf.getText();  //입력한 텍스트를 읽어서 msg에 저장
			ta.append(msg +"\n"); // msg를 보여줌
			if(msg.equals("exit")){
				ta.append("bye");
				stop = true;
